...
analyzed_filename="${core_dir}/data/${month}/${current_date}-Data/pine_picks_analyzed_${current_date_snake_case}.csv"
hits_filename="${core_dir}/data/hits.txt"

python3 "$scripts_dir/process_pine_picks.py" "$box_score_filename" "$pine_picks_filename" "$analyzed_filename"
python3 "$scripts_dir/count_pine_hits.py" "$analyzed_filename" "$hits_filename"
